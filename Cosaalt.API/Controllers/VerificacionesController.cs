...
public class VerificacionesController : ControllerBase
{
    ...
    public async Task<IActionResult> Dashboard(int idMecanico)
    {
        var result = await _verificacionService.ObtenerDashboardAsync(idMecanico);
        return Ok(result);
    }

    [HttpGet("historial/{idMecanico}")]
    public async Task<IActionResult> Historial(int idMecanico, [FromQuery] DateTime? desde, [FromQuery] DateTime? hasta, [FromQuery] string? estado, [FromQuery] string? resultado, [FromQuery] string? buscar, [FromQuery] int page = 1, [FromQuery] int pageSize = 50)
    {
        var filtro = new VerificacionHistorialFiltro(desde, hasta, estado, resultado, buscar, page, pageSize);
        var result = await _verificacionService.ObtenerHistorialAsync(idMecanico, filtro);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> GuardarParticipantes(int id, [FromBody] IReadOnlyList<ParticipanteVerificacionDto> request)
    {
        try
        {
            var result = await _verificacionService.GuardarParticipantesAsync(id, request);
            return result is null ? NotFound() : Ok(result);
        }
        catch (InvalidOperationException ex)
        {
    ...
    [HttpPost("calcular")]
    public async Task<IActionResult> Calcular([FromBody] GuardarEnsayoRequestDto request)
    {
        var result = await _verificacionService.CalcularAsync(request);
        return Ok(result);
    }
    ...
    public async Task<IActionResult> Finalizar(int id)
    {
        try
        {
            var result = await _verificacionService.FinalizarAsync(id);
            return result is null ? NotFound() : Ok(result);
        }
        catch (InvalidOperationException ex)
        {
    ...
    [HttpGet("{id}/informes")]
    public async Task<IActionResult> Informes(int id)
    {
        var result = await _verificacionService.ObtenerInformesAsync(id);
        return Ok(result);
    }

    [HttpPost("{id}/informe")]
    public async Task<IActionResult> GenerarInforme(int id, [FromBody] GenerarInformeRequestDto? request)
    {
        try
        {
            request ??= new GenerarInformeRequestDto(null, null);
            var result = await _verificacionService.GenerarInformeAsync(id, request);
            return Ok(result);
        }
        catch (InvalidOperationException ex)
        {
